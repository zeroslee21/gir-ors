...
**데이터 의존성**:
- data/methodologies.json (전체 314개)
...
//       /methodologies/CDM-METHODOLOGY-002.html
//       ... (314개)
```
...
**데이터 의존성**:
- data/methodologies.json (전체)
- data/sectoral-scopes.json (섹터 정보)
- data/governing-bodies.json (정부기관)

**빌드 타임 동작**:
1. 314개 방법론에 대해 `generateStaticParams()` 호출
2. 각각에 대해 정적 HTML 생성
3. 약 314개의 `.html` 파일 생성
4. 빌드 시간: 약 30-60초
...
**데이터 의존성**:
- data/statistics.json (미리 계산된 통계)
- data/methodologies.json
...
| 방법론 상세 | SSG | generateStaticParams | 314 |
| 섹터 목록 | SSG | 수동 | 1 |
| 섹터 상세 | SSG | generateStaticParams | ~20 |
| 통계 | SSG | 수동 | 1 |
| 가이드 | SSG | 수동 | 1 |
| **총합** | | | **약 338개** |
...
2. **페이지 수집**: app/ 디렉토리 스캔
3. **동적 페이지 생성**:
   - methodologies/[slug]/page.tsx → 314개 HTML
   - sectors/[slug]/page.tsx → ~20개 HTML
4. **정적 페이지 생성**: 나머지 페이지
5. **번들링**: 공통 JavaScript 번들 생성
6. **최적화**: 이미지, CSS 최적화
7. **산출**: `.next/static/` 에 결과물 저장
